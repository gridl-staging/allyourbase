...
assert_section_contains _dev/PHASES.md "### Later (important but not urgent)" "Realtime coverage in this lane remains WebSocket-only and SSE automation stays deferred." "phase tracking should preserve websocket-only realtime scope and SSE deferral in summary form"
assert_section_contains _dev/PHASES.md "### Later (important but not urgent)" "Remaining load confidence work is successful multi-hour soak execution and pooled-session rollover validation beyond 10 minutes." "phase tracking should preserve long-soak and pooled-session rollover gaps in summary form"
