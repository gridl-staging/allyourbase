...
assert_section_contains _dev/FEATURES.md '### Testing Infrastructure Gaps (Phase 5 — see `_dev/PHASES.md`)' 'Load/stress harness now has self-bootstrapping local wrappers plus first real scale-tier evidence' "features tracking should summarize the shipped tier alias coverage while pointing to the canonical baseline evidence log"
...
assert_section_contains _dev/FEATURES.md '### Testing Infrastructure Gaps (Phase 5 — see `_dev/PHASES.md`)' "Remaining load gaps are successful multi-hour soak windows beyond the 12-minute rollover proof and follow-up triage on the observed tier failures." "features tracking should preserve the remaining long-soak and scale-tier follow-up gaps in summary form"
